...
uint8_t vec_size = 0;
new_event ehqueue[EHQUEUESZ] = {0, 0};
...
int8_t register_handler(uint8_t event_id, uint32_t handler, char *evname, void*, uint8_t) {
  ...
  for (selected = 0; selected < EVENTQTTY; selected++) {
    if (ehvecpointers[selected].id == event_id && !strcmp(ehvecpointers[selected].name, evname))
      break; // Selected is now the position of the event on the vector
  }
  if (selected == EVENTQTTY) // Case where there was no break, thus no match was found
  {
    ...
    {
      // printf("\t\t\t\t\t***\t\t\tSelected is now the position of the first
      // empty slot on the array\n");
      if (ehvecpointers[selected].id == -1)
        break; // Selected is now the position of the first empty slot on the
               // array
    }
    ...
    for (ehvecpointers[selected].pos = 0; ehvecpointers[selected].pos < EHVECSZ; ehvecpointers[selected].pos++) {
      // printf("\t\t\t\t\t***\t\t\tfoooorrr....\n");
      if (ehvec[ehvecpointers[selected].pos] == 0)
        break;
    } // Gets the first free position for the new event
  }
  // printf("****%d\n****%d\n****%d\n****%d\n", selected,
  // ehvecpointers[selected].pos, ehvecpointers[selected].sz,
  // ehvec[ehvecpointers[selected].pos + ehvecpointers[selected].sz]);
  if (ehvec[ehvecpointers[selected].pos + ehvecpointers[selected].sz] ==
      0) // No handler in the position after the last handler of this event
  {
    // printf("\t\t\t\t\t***\t\t\tNo handler in the position after the last
  ...
  } else // Shift to get space
  {
    ...
    for (first_zero = 0; first_zero < EHVECSZ; first_zero++) {
      if (ehvec[first_zero] == 0)
        break;
    } // find first empty, check which event it is from, then shift...
      // firstzero>pos and <= pos+sz
    if (first_zero < ehvecpointers[selected].pos) // the empty space is before the event, should shift left
    {
      ...
      {
        if (first_zero < ehvecpointers[idnearfz].pos + ehvecpointers[idnearfz].sz - 1)
          break; // Ensures that the first zero is on that id
      }
      ...
      {
        if (ehvecpointers[idnearfz].id == -1)
          continue; // Position not used
        if (first_zero > ehvecpointers[idnearfz].pos + ehvecpointers[idnearfz].sz - 1)
          break; // Ensures that the first zero is on that id
      }
      for (; idnearfz > selected; idnearfz--) // Starts shifting id by id
  ...
  /*	// CREATION
          if (!strcmp(evname, "ROPR")) //US dist sensor has the threshold
     argument
          {
                  printf("Limiar (robot_proximit_threshold) set to %d\n",
     (uint8_t)RF[7]); // threshold is in $a3 robot_proximit_threshold =
     (uint8_t)RF[7];
          }
          else if (!strcmp(evname, "RESE")) //US dist sensor has the threshold
     argument
          {
                  printf("Limiar (robot_timer_threshold) set to %d\n",
     (uint8_t)RF[7]); // threshold is in $a3 robot_timer_threshold =
     (uint8_t)RF[7];
          }
          else if (!strcmp(evname, "ROTA")) //room temperature Above
          {
                  printf("Limiar (room_temperature_above_threshold) set to
     %d\n", (uint8_t)RF[7]); // threshold is in $a3
                  room_temperature_above_threshold = (uint8_t)RF[7];
          }
          else if (!strcmp(evname, "ROTB")) //room below
          {
                  printf("Limiar (room_temperature_below_threshold) set to
     %d\n", (uint8_t)RF[7]); // threshold is in $a3
  ...
  // TODO:Remember to tell HAL to generate events! -- Currently it always
  // generate events
...
int8_t remove_handler(uint8_t event_id, uint32_t handler, char *evname)
{
  ...
  for (event = ehvecpointers[selected].pos; event < ehvecpointers[selected].sz; event++) {
    if (ehvec[event] == handler)
      break;
  }
  if (ehvec[event] != handler)
    return -1; // No such handler

  ehvec[event] = ehvec[ehvecpointers[selected].pos + ehvecpointers[selected].sz -
                       1]; // Make sure there won`t be a gap in the vector
