...
// **************** libecc/include/mips/bits/alltypes.h **************************
...
// **************** libecc/include/mips/bits/float.h **************************

#define LDBL_EPSILON 2.22044604925031308085e-16L

// ********************* libecc/include/float.h *******************************

#define DBL_EPSILON 2.22044604925031308085e-16

// ********************* libecc/include/math.h *******************************
...
#endif
static const double_t toint_floor = 1 / EPS; // =========== Changed toint to toint_floor to avoid conflict. ===========
 ...
 * pio2_1t:  pi/2 - pio2_1
 */
static const double toint___rem_pio2f =
                        1.5 / EPS, // =========== Changed toint to toint___rem_pio2f to avoid conflict. ===========
...
int __rem_pio2f(float x, double *y)
{
  ...
  if (ix < 0x4dc90fdb) { /* |x| ~< 2^28*(pi/2), medium size */
    ...
    fn = (double_t)x * invpio2 + toint___rem_pio2f - toint___rem_pio2f;
    n = (int32_t)fn;
...
cplx cplx_exp(cplx z)
{
  float imag_sin, imag_cos, real_exp;
  cplx c;

  imag_sin = sinf(z.imag);
  imag_cos = cosf(z.imag);
  real_exp = expf(z.real);

  c.real = real_exp * imag_cos;
  c.imag = real_exp * imag_sin;
...
void show(char *s)
{
  ...
  for (i = 0; i < WINDOW_SIZE; ++i) {
    if (!values[i].imag) {
      print_float((uint32_t) & (values[i].real), 5);
      print_string(" ");
    } else {
      print_string("(");
      print_float((uint32_t) & (values[i].real), 5);
      print_string(", ");
      print_float((uint32_t) & (values[i].imag), 5);
      print_string(") ");
    }
  }
...
void _fft(cplx buf[], cplx out[], int n, int step)
{
  if (step < n) {
    ...
    for (i = 0; i < n; i += 2 * step) {
      ...
      t = cplx_mult(t, out[i + step]);

      buf[i / 2] = cplx_add(out[i], t);
      buf[(i + n) / 2] = cplx_sub(out[i], t);
    }
  }
}
...
void fft()
{
  ...
  _fft(values, out, WINDOW_SIZE, 1);
}

void event_handler()
{
  float number;
  readFloatState("sensor.sensors.temperature", STATE_CURRENT_VALUE, &number);

  values[window_counter].real = number;
  values[window_counter].imag = 0.0f;
