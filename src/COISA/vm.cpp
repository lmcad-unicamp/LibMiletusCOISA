...
// RF[rt] = ((uint32_t)immediate) << 16;
  ...
  case 0b100000: { // lb      100000  LoadStore       $t = SE (MEM [$s + i]:1)
    ...
    fprintf(stderr, "lb\n");
#endif
    RF[rt] = ((uint32_t)VM_memory[RF[rs] + immediate] & 0x7F) | (uint32_t)(VM_memory[RF[rs] + immediate] & 0x80)
                                                                    << 24; // Load byte carrying signal to the register
    break;
  }
...
#endif
    RF[rt] =
        ((((uint32_t)(VM_memory[RF[rs] + immediate]) << 16) | ((uint32_t)VM_memory[RF[rs] + immediate + 1])) & 0x7FFF) |
        (uint32_t)((VM_memory[RF[rs] + immediate + 1]) & 0x8000) << 16;
    break;
  }
...
#endif
    RF[rt] =
        ((((uint32_t)(VM_memory[RF[rs] + immediate]) << 16) | ((uint32_t)VM_memory[RF[rs] + immediate + 1])) & 0xFFFF);
    break;
  }
...
#endif
    RF[rt] =
        ((uint32_t)(VM_memory[RF[rs] + immediate + 0]) << 24) | ((uint32_t)(VM_memory[RF[rs] + immediate + 1]) << 16) |
        ((uint32_t)(VM_memory[RF[rs] + immediate + 2]) << 8) | ((uint32_t)(VM_memory[RF[rs] + immediate + 3]) << 0);
    break;
  }
  ...
  case 0b011010: { // trap    011010  Trap    Dependent on operating system; different values for immed26 specify
                   // different operations. See the list of traps for information on what the different trap codes do.
#if DEBUGGING
    fprintf(stderr, "trap\n");
#endif
  ...
  case 0x11: {
    ...
    case 0x11: {
      ...
      case 0x0: { // add.d fd, fs, ft
        ...
        num_fs.as_int64 = (((uint64_t)tmp_fs0.as_int32) << 32) | (((uint64_t)tmp_fs1.as_int32) << 0);

        num_ft.as_int64 = (((uint64_t)tmp_ft0.as_int32) << 32) | (((uint64_t)tmp_ft1.as_int32) << 0);
  ...
  case 0x13: {
    ...
    case 0x0: { // lwxc1 fd, index(base) (index = ft, base = fr)
      ...
      num.as_int32 =
          ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 0]) << 24) | ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 1]) << 16) |
          ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 2]) << 8) | ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 3]) << 0);
    ...
    case 0x1: { // ldxc1 fd, index(base) (index = ft, base = fr)
      ...
      num.as_int32 =
          ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 4]) << 24) | ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 5]) << 16) |
          ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 6]) << 8) | ((uint32_t)(VM_memory[RF[fr] + RF[ft] + 7]) << 0);
    ...
    case 0x21: { // madd.d fd, fr, fs, ft
      ...
      num_fr.as_int64 = (((uint64_t)tmp_fr0.as_int32) << 32) | (((uint64_t)tmp_fr1.as_int32) << 0);
  ...
  case 0x35: { // ldc1 ft, address
    ...
    num.as_int32 =
        ((uint32_t)(VM_memory[RF[rs] + immediate + 4]) << 24) | ((uint32_t)(VM_memory[RF[rs] + immediate + 5]) << 16) |
        ((uint32_t)(VM_memory[RF[rs] + immediate + 6]) << 8) | ((uint32_t)(VM_memory[RF[rs] + immediate + 7]) << 0);
